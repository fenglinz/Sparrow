...
<xsl:call-template name="properties" />
  ...
  <xsl:template name="properties">
    <xsl:for-each select="./table/column">
        ...
        /// <![CDATA[</summary>]]>
        [Column("<xsl:value-of select="@name"/>"<xsl:if test="@isPrimaryKey='true'">, IsPrimaryKey = true</xsl:if><xsl:if test="@isNewGuid='true'">, IsNewGuid = true</xsl:if><xsl:if test="@nullable='true'">, IsNullable = true</xsl:if>)]
        <xsl:if test="@nullable='false'">[Required(AllowEmptyStrings = false, ErrorMessage = "<xsl:value-of select="@description" />不能为空！")]
        </xsl:if>
