...
<xsl:stylesheet version="1.0" xmlns="http://www.csbr.com/CommandText.xsd" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="root">
    <sqlMap xmlns="http://www.csbr.com/CommandText.xsd">
  ...
  <xsl:template name="createOrUpdate">
    ...
    <update id="CreateOrUpdate">
      ...
      MERGE INTO <xsl:value-of select="./table/@name" /> a
      USING(SELECT * FROM <xsl:value-of select="./table/@name" /> WHERE <xsl:choose><xsl:when test="count(./table/column[@isPrimaryKey='true'])=1"><xsl:value-of select="./table/column[@isPrimaryKey='true'][1]/@name"/>=#<xsl:value-of select="./table/column[@isPrimaryKey='true'][1]/@propertyName" />#</xsl:when>
        <xsl:otherwise>
          ...
          </xsl:for-each></xsl:otherwise></xsl:choose>) b
      ON (<xsl:choose><xsl:when test="count(./table/column[@isPrimaryKey='true'])=1">a.<xsl:value-of select="./table/column[@isPrimaryKey='true'][1]/@name"/>=b.<xsl:value-of select="./table/column[@isPrimaryKey='true'][1]/@propertyName" />
        </xsl:when>
        <xsl:otherwise>
          <xsl:for-each select="./table/column[@isPrimaryKey='true']">
    ...
    <select id="GetById">
      ...
      <xsl:for-each select="./table/column">
        <xsl:if test="position()=1">
          ...
          <xsl:value-of select="@name"/>
          <xsl:text> AS "</xsl:text><xsl:value-of select="@propertyName"/>"<xsl:if test="position()!=last()"><xsl:text>,
          </xsl:text></xsl:if>
      </xsl:for-each>
  ...
  <xsl:template name="search">
    ...
    <select>
      ...
      <xsl:for-each select="./table/column">
      <xsl:text>      </xsl:text><xsl:value-of select="@name"/> AS "<xsl:value-of select="@propertyName"/>",
      </xsl:for-each>
      <xsl:text>      ROW_NUMBER() OVER(ORDER BY </xsl:text><xsl:choose>
        <xsl:when test="count(./table/column[@isPrimaryKey='true'])=1">
          <xsl:for-each select="./table/column[@isPrimaryKey='true']">
        ...
        <xsl:when test="count(./table/column[@isPrimaryKey='true'])>1">
          ...
          </xsl:for-each></xsl:when></xsl:choose>) AS "RowIndex"
          FROM <xsl:value-of select="./table/@name" />
      <xsl:text>
      </xsl:text>
      <include refid="searchConditions" />
